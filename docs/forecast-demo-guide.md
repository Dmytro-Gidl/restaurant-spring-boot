...
2. Apply the SQL in `docs/forecast-demo.sql` (e.g., via `psql`, `mysql`, or your IDE's console). It truncates core tables **and cached forecast rows** so you start clean, then seeds three dishes and users, and inserts **15 months (Jan 2024–Mar 2025)** of order history so every model has multiple validation folds.
...
### Trend Taco (dish 100)
*History:* Quantities climb linearly from 8 to 64 over fifteen months.

| Model | Expected behavior | What to point out |
|-------|-------------------|-------------------|
| Holt–Winters | Projects the rising trend, giving ~68–70 portions for the next month and showing lower MAPE/RMSE thanks to the longer series. | Trend sensitivity and smooth growth; highlight the tuned smoothing parameters in the “Details” panel. |
...
*History:* Alternating high/low months (e.g., 38, 12, 36, 14 … with a mid-20s drift into 2025).

| Model | Expected behavior | What to point out |
|-------|-------------------|-------------------|
| Holt–Winters | Dampens oscillations toward the mid-20s rather than chasing each swing. | Its RMSE stays modest because smoothing reduces overreaction, and now the metrics use several folds. |
...
*History:* Only three non-zero months spread out.
